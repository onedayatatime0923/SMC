...
def smt_real(value, use_scientific_notation=True):
    try:
        value = float(value)
    except TypeError:
        value = float(value[0])
    ...
    if abs(value - round(value)) < 1e-12:
        ...
        if not use_scientific_notation and "e" in text.lower():
...
def smt_mul_real(coeff, var, use_scientific_notation=True):
    coeff = float(coeff)
    if abs(coeff) < 1e-12:
        return "0"
    ...
    if abs(coeff + 1.0) < 1e-12:
        return "(- %s)" % var
    return "(* %s %s)" % (smt_real(coeff, use_scientific_notation), var)


def smt_linear(coeffs, vars, use_scientific_notation=True):
    return smt_add([
        smt_mul_real(coeff, var, use_scientific_notation)
        for coeff, var in zip(coeffs, vars)
    ])


def smt_linear_atom(coeffs, vars, rhs, sense, use_scientific_notation=True):
    op = {"L": "<=", "G": ">=", "E": "="}.get(sense, "<=")
    return "(%s %s %s)" % (
        op,
        smt_linear(coeffs, vars, use_scientific_notation),
        smt_real(rhs, use_scientific_notation),
    )
