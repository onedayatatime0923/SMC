...
#ifndef NNV__env__mountain_car_hpp
#define NNV__env__mountain_car_hpp
...
class CMountainCar: public CEnv{
public:
    struct Parameter {
        pair<double, double> position_bound = make_pair(-1.2, 0.6);
        pair<double, double> velocity_bound = make_pair(-0.07, 0.07);
        pair<double, double> position_initial_constraint = make_pair(-0.11, -0.1);
        // pair<double, double> position_initial_constraint = make_pair(-0.5, -0.5);
        // pair<double, double> position_initial_constraint = make_pair(0.48, 0.48);
        pair<double, double> velocity_initial_constraint = make_pair(0, 0);
        // pair<double, double> velocity_initial_constraint = make_pair(0.03, 0.03);
        double safe_constraint          = 0.5;
        double perception_disturbance   = 0.009;
    } parameter_;

    CMountainCar(CSolver& solver) : CEnv(solver) {};
    ...
    vector<GRBVar>              StateVar        (int step) { return {time_state_var_vv_[step][0], time_state_var_vv_[step][3]}; }
    int                         Action          (int step);
